...
page(tr("Install/Update Languages"));
...
function display_language_edit($selected_id)
{
	...
	text_row_ex(tr("Language"), 'code', 20);
	text_row_ex(tr("Name"), 'name', 20);
	text_row_ex(tr("Encoding"), 'encoding', 20);

	yesno_list_row(tr("Right To Left"), 'rtl', null, "", "", false);

	label_row(tr("Language File") . " (PO)", "<input name='uploadfile' type='file'>");
	label_row(tr("Language File") . " (MO)", "<input name='uploadfile2' type='file'>");

	end_table(0);
	display_note(tr("Select your language files from your local harddisk."), 0, 1);
	echo "<center><input onclick='javascript:updateLanguage()' type='button' style='width:150' value='". tr("Save"). "'>";
...
hyperlink_no_params($_SERVER['PHP_SELF'], tr("Create a new language"));
