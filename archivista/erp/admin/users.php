...
{
	display_notification_centered(tr("A new user has been added."));
}

if (isset($_GET['UpdatedID'])) 
{
    display_notification_centered(tr("The selected user has been updated."));
}

if (isset($_GET['DeletedID'])) 
{
	display_notification_centered(tr("User has been deleted."));
}
...
function can_process() 
{
	...
	{
		display_error( tr("The user login entered must be at least 4 characters long."));
		set_focus('user_id');
		return false;
	}
	...
	if ($_POST['password'] != "") 
	{
    	...
    	{
    		display_error( tr("The password entered must be at least 4 characters long."));
		set_focus('password');
    		return false;
    	}
...
if ($_SESSION["wa_current_user"]->access == 2)
	$th = array(tr("User login"), tr("Full Name"), tr("Phone"),
		tr("E-mail"), tr("Last Visit"), tr("Access Level"), "", "");
else		
	$th = array(tr("User login"), tr("Full Name"), tr("Phone"),
		tr("E-mail"), tr("Last Visit"), tr("Access Level"), "");
table_header($th);	
...
if (isset($selected_id)) 
{
	...
	start_row();
	label_row(tr("User login:"), $_POST['user_id']);

} 
else 
{ //end of if $selected_id only do the else when a new record is being entered
	text_row(tr("User Login:"), "user_id",  null, 22, 20);
}
$_POST['password'] = "";
start_row();
label_cell(tr("Password:"));
label_cell("<input type='password' name='password' size=22 maxlength=20 value='" . $_POST['password'] . "'>");
end_row();

if (isset($selected_id)) 
{
	table_section_title(tr("Enter a new password to change, leave empty to keep current."));
}

text_row_ex(tr("Full Name").":", 'real_name',  50);

text_row_ex(tr("Telephone No.:"), 'phone', 30);

text_row_ex(tr("Email Address:"), 'email', 50);

security_headings_list_row(tr("Access Level:"), "Access", null); 

languages_list_row(tr("Language:"), 'language', null);
