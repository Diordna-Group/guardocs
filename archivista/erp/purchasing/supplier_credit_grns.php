...
{
	display_note(tr("To enter supplier transactions the supplier must first be selected from the supplier selection screen, then the link to enter a supplier credit note must be clicked on."));
	exit;
...
hyperlink_no_params("$path_to_root/purchasing/supplier_credit.php", tr("Return to Credit Note Entry"));
echo "<hr>";
...
{
	display_note(tr("There are no received items for the selected supplier that have been invoiced."));
	display_note(tr("Credits can only be applied to invoiced items."));
...
$th = array(tr("Delivery"), tr("Sequence #"), tr("Order"), tr("Item Code"), tr("Description"),
	tr("Delivered"), tr("Total Qty Received"), tr("Qty Already Invoiced"),
	tr("Qty Yet To Invoice"), tr("Order Price"), tr("Line Value"));
table_header($th);
$i = $k =0;
...
if (isset($_POST['grn_item_id']) && $_POST['grn_item_id'] != "")
{
	...
	$th = array(tr("Sequence #"), tr("Item"), tr("Qty Already Invoiced"),
		tr("Quantity to Credit"), tr("Order Price"), tr("Credit Price"));
	table_header($th);	
