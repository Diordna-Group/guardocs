...
page(tr("Bank Transaction Types"));
...
if (isset($_POST['ADD_ITEM']) || isset($_POST['UPDATE_ITEM'])) 
{
	...
	{
		$input_error = 1;
		display_error( tr("The bank transaction type name cannot be empty."));
		set_focus('name');
	}
...
function can_delete($selected_id)
{
	if ($selected_id == -1)
		return false;
	$sql= "SELECT COUNT(*) FROM bank_trans WHERE bank_trans_type_id=$selected_id";
	$result = db_query($sql, "could not query bank transactions");
	$myrow = db_fetch_row($result);
	if ($myrow[0] > 0) 
	{
		display_error(tr("Cannot delete this bank transaction type because bank transactions have been created referring to it."));
		return false;
	}
...
hyperlink_no_params($_SERVER['PHP_SELF'], tr("New Bank Transaction Type"));
