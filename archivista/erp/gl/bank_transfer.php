...
page(tr("Transfer between Bank Accounts"), false, false, "", $js);

check_db_has_bank_accounts(tr("There are no bank accounts defined in the system."));
check_db_has_bank_trans_types(tr("There are no bank transfer types defined in the system."));
...
if (isset($_GET['AddedID'])) 
{
	...
	$trans_type = systypes::bank_transfer();
   	...
   	display_notification_centered( tr("Transfer has been entered"));

	display_note(get_gl_view_str($trans_type, $trans_no, tr("View the GL Journal Entries for this Transfer")));

   	hyperlink_no_params($_SERVER['PHP_SELF'], tr("Enter Another Transfer"));
