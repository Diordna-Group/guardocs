...
page(tr("Journal Entry"), false, false,'', $js);
...
if (isset($_GET['AddedID'])) 
{
	...
	$trans_type = systypes::journal_entry();
   	...
   	display_notification_centered( tr("Journal entry has been entered") . " #$trans_no");

    display_note(get_gl_view_str($trans_type, $trans_no, tr("View this Journal Entry")));

   	hyperlink_no_params($_SERVER['PHP_SELF'], tr("Enter Another Journal Entry"));
...
start_row();
echo "<td>";
display_gl_items(tr("Rows"), $_SESSION['journal_items']);
gl_options_controls();
echo "</td>";
end_row();
end_table(1);
...
{
    submit_center('Process', tr("Process Journal Entry"));
} 
else 
{
	display_note(tr("The journal must balance (debits equal to credits) before it can be processed."), 0, 1);
}
