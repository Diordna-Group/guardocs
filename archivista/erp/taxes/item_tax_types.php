...
if (isset($_POST['ADD_ITEM']) || isset($_POST['UPDATE_ITEM'])) 
{
	...
	{
		$input_error = 1;
		display_error(tr("The item tax type description cannot be empty."));
		set_focus('name');
	}
...
{
	$sql= "SELECT COUNT(*) FROM stock_master WHERE tax_type_id=$selected_id";
	$result = db_query($sql, "could not query stock master");
	$myrow = db_fetch_row($result);
	if ($myrow[0] > 0) 
	{
		display_error(tr("Cannot delete this item tax type because items have been created referring to it."));
		return false;
	}
...
$th = array(tr("Name"), tr("Tax exempt"),'','');
...
hyperlink_no_params($_SERVER['PHP_SELF'], tr("New Item Tax type"));
...
yesno_list_row(tr("Is Fully Tax-exempt:"), 'exempt', null, "", "", true);
...
if (!isset($_POST['exempt']) || $_POST['exempt'] == 0) 
{
    ...
    display_note(tr("Select which taxes this item tax type is exempt from."), 0, 1);
    
    start_table($table_style2);
    $th = array(tr("Tax Name"), tr("Rate"), tr("Is exempt"));
    table_header($th);
