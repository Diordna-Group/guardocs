...
page(tr("Work Order Entry"), false, false, "", $js);


check_db_has_manufacturable_items(tr("There are no manufacturable items defined in the system."));
...
if (isset($_GET['AddedID']))
{
	...
	display_notification_centered(tr("The work order been added."));
...
if (isset($_GET['UpdatedID']))
{
	...
	display_notification_centered(tr("The work order been updated."));
	safe_exit();
}
...
if (isset($_GET['DeletedID']))
{
	...
	display_notification_centered(tr("Work order has been deleted."));
	safe_exit();
}
...
if (isset($_GET['ClosedID']))
{
	...
	display_notification_centered(tr("This work order has been closed. There can be no more issues against it.") . " #$id");
	safe_exit();
}
        	...
        	if ($_POST['type'] == wo_types::assemble())
        	{
            	...
            	while ($bom_item = db_fetch($result))
            	{
            		...
            		if (has_stock_holding($bom_item["ResourceType"]))
            		{
                		...
                		if (-$quantity + $qoh < 0)
                		{
                			display_error(tr("The work order cannot be processed because there is an insufficient quantity for component:") .
                				" " . $bom_item["component"] . " - " .  $bom_item["description"] . ".  " . tr("Location:") . " " . $bom_item["location_name"]);
						set_focus('quantity');
        					return false;
                		}
            		}
...
if (isset($_POST['delete']))
{
	...
	{
		display_error(tr("This work order cannot be deleted because it has already been processed."));
		$cancel_delete = true;
	}
...
if (isset($selected_id))
{
	...
	{
		echo "<center>";
		display_error(tr("This work order is closed and cannot be edited."));
		safe_exit();
	}
