...
if (isset($_POST['ADD_ITEM']) || isset($_POST['UPDATE_ITEM'])) 
{
	...
	{
		$input_error = 1;
		display_error(tr("The work centre name cannot be empty."));
		set_focus('name');
	}
...
{
	$sql= "SELECT COUNT(*) FROM bom WHERE workcentre_added='$selected_id'";	
	$result = db_query($sql, "check can delete work centre");
	$myrow = db_fetch_row($result);
	if ($myrow[0] > 0) 
	{
		display_error(tr("Cannot delete this work centre because BOMs have been created referring to it."));
		return false;
	}
	
	$sql= "SELECT COUNT(*) FROM wo_requirements WHERE workcentre='$selected_id'";
	$result = db_query($sql, "check can delete work centre");
	$myrow = db_fetch_row($result);
	if ($myrow[0] > 0) 
	{
		display_error(tr("Cannot delete this work centre because work order requirements have been created referring to it."));
		return false;
	}		
