...
function can_process() 
{
	...
	{
		display_error(tr("The credit status description cannot be empty."));
		return false;
	}	
...
function can_delete($selected_id)
{
	...
	if ($myrow[0] > 0) 
	{
		display_error(tr("Cannot delete this credit status because customer accounts have been created referring to it."));
		return false;
	}
...
$th = array(tr("Description"), tr("Dissallow Invoices"),'','');
table_header($th);
...
while ($myrow = db_fetch($result)) 
{
	...
	{
		$disallow_text = tr("Invoice OK");
	} 
	else 
	{
		$disallow_text = "<b>" . tr("NO INVOICING") . "</b>";
	}
...
hyperlink_no_params($_SERVER['PHP_SELF'], tr("New Credit Status"));
...
text_row_ex(tr("Description:"), 'reason_description', 50);

yesno_list_row(tr("Dissallow invoicing ?"), 'DisallowInvoices', null); 
