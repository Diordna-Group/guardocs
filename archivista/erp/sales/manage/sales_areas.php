...
if (isset($_POST['ADD_ITEM']) || isset($_POST['UPDATE_ITEM'])) 
{
	...
	{
		$input_error = 1;
		display_error(tr("The area description cannot be empty."));
	}
	...
	if ($input_error != 1)
	{
    	...
    	{
    		$sql = "UPDATE areas SET description=".db_escape($_POST['description'])." WHERE area_code = '$selected_id'";
    	} 
    	else 
    	{
...
if (isset($_GET['delete'])) 
{
	...
	$sql= "SELECT COUNT(*) FROM cust_branch WHERE area='$selected_id'";
	$result = db_query($sql,"check failed");
	$myrow = db_fetch_row($result);
	if ($myrow[0] > 0) 
	{
		$cancel_delete = 1;
		display_error(tr("Cannot delete this area because customer branches have been created using this area."));
	} 
	if ($cancel_delete == 0) 
	{
		$sql="DELETE FROM areas WHERE area_code='" . $selected_id . "'";
...
text_row_ex(tr("Area Name:"), 'description', 30); 
