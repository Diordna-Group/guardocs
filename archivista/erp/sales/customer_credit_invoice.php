...
if (isset($_GET['ModifyCredit'])) {
	$_SESSION['page_title'] = sprintf(tr("Modifying Credit Invoice # %d."), $_GET['ModifyCredit']);
	$help_page_title =tr("Modifying Credit Invoice");
	processing_start();
} elseif (isset($_GET['InvoiceNumber'])) {
	$_SESSION['page_title'] = tr("Credit all or part of an Invoice");
	processing_start();
}
...
//	sales_types_list_cells(null, 'sales_type_id', $_POST['sales_type_id']);
    ...
    $th = array(tr("Item Code"), tr("Item Description"), tr("Invoiced Quantity"), tr("Units"),
    	tr("Credit Quantity"), tr("Price"), tr("Discount %"), tr("Total"));
    table_header($th);
...
echo "&nbsp";
submit('ProcessCredit', tr("Process Credit Note"));
echo "</center>";
