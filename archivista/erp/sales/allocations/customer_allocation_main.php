...
page(tr("Customer Allocations"), false, false, "", $js);
...
function display_allocatable_transactions()
{
	...
	if (!isset($_POST['customer_id']))
		$th = array(tr("Transaction Type"), tr("#"), tr("Reference"), tr("Date"), tr("Customer"),
			tr("Currency"), tr("Total"), tr("Left To Allocate"), "");
	else
		$th = array(tr("Transaction Type"), tr("#"), tr("Reference"), tr("Date"),
			tr("Total"), tr("Left To Allocate"), "");
	table_header($th);
	...
	while ($myrow = db_fetch($trans_items))
	{
    	...
    	if ($myrow["Total"] - $myrow["alloc"] != 0.0)
    		label_cell("<a href='$path_to_root/sales/allocations/customer_allocate.php?trans_no="
					.$myrow["trans_no"] . "&trans_type=" . $myrow["type"]  . "'>" . tr("Allocate") . "</a>");
    	else
    		label_cell("");
    	end_row();
	}
