...
page(tr("Item Adjustments Note"), false, false, "", $js);

//-----------------------------------------------------------------------------------------------

check_db_has_costable_items(tr("There are no inventory items defined in the system which can be adjusted (Purchased or Manufactured)."));
...
if (isset($_GET['AddedID'])) 
{
	...
	display_notification_centered(tr("Items adjustment has been processed"));
	display_note(get_trans_view_str($trans_type, $trans_no, tr("View this adjustment")));

	display_note(get_gl_view_str($trans_type, $trans_no, tr("View the GL Postings for this Adjustment")), 1, 0);

	hyperlink_no_params($_SERVER['PHP_SELF'], tr("Enter Another Adjustment"));
...
function can_process()
{
	...
	if ($failed_item != null) 
	{
    	display_error(tr("The adjustment cannot be processed because an adjustment item would cause a negative inventory balance :") .
...
function check_item_data()
{
	if (!check_num('qty',0))
	{
		display_error(tr("The quantity entered is negative or invalid."));
		set_focus('qty');
		return false;
	}
...
start_row();
echo "<TD>";
display_adjustment_items(tr("Adjustment Items"), $_SESSION['adj_items']);
...
{
    submit_cells('Process', tr("Process Adjustment"));
}
end_row();
end_table();
