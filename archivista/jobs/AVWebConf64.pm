...
use constant CHECK_OK => 2; # check if action was sucessfully (seconds)
...
sub new {
  ...
  my $self = {};
	bless $self,$class;
  $self->session(AVSession64->new("webconfig"));
...
sub doAction {
  my $self = shift;
    ...
    if ($self->session->cookie && $self->cgi) {
		  $loggedin = 1;
  ...
  my ($self,$mode,$nextaction,$pparam,$ppempty) = @_;
	...
	if ($doit==1) {
      ...
      foreach my $key (keys %{$pparam}) {
			  my $qvalue = $self->session->param($key);
				my $qkey = $$pparam{$key};
				$out .= "$qkey=$qvalue\n";
			}
	    my ($fh,$file) = tempfile("/etc/webconfig/c-XXXX");
			open(FOUT,">>/tmp/log.txt");
			print FOUT $out;
			close(FOUT);
			print $fh $out;
			close($fh);
...
sub getMainLogFile {
  my ($self) = @_;
	...
	if ($self->form eq STATUS) {
	  $res = `sudo /home/archivista/status.sh -webconfig`;
		$rows = 30;
		$reverse = 0;
...
sub getMainSettings {
  ...
  my $gwon = 0;
	my $nson = 0;
	my $ip = `sudo /etc/perl/install.pl getip`;
	my $cidr = `sudo /etc/perl/install.pl getcidr`;
	$ip = "$ip$cidr";
	my $gw = `sudo /etc/perl/install.pl getgw`;
	my $ns = `sudo /etc/perl/install.pl getdns`;
	$gwon=1 if $ip eq "";
	$gwon=1 if $ip ne "" && $gw ne "";
	$nson=1 if $ip eq "";
	$nson=1 if $ip ne "" && $ns ne "";
	my $css_class='';
...
sub getMainBackupNow {
  my ($self) = @_;
	...
	my (%files,%labels);
	%files=(usb=>F_USB, network=>F_NET, rsync=>F_RSYNC);
	%labels=(usb=>$self->cgi->string(USB),network=>$self->cgi->string(NETBACKUP),
	      rsync=>$self->cgi->string(RSYNC));
	my $default = 'usb';
	my (@radios,%elements);
	my @types = ("usb","network","rsync");
	foreach my $id (@types) {
	  my $res=`grep 'days=' $files{$id}`; # Check if days are set
		chomp $res;
		if ($res ne "" || $id eq 'usb') { # Backup possible?
