  ...
  while ( $aktnr <= $endnr ) {
    ...
    if ($akte>0) {
      ...
      for ( my $c = 1 ; $c <= $seiten ; $c++ ) {
        ...
        addImageToDB($dbh,$db1,$akte,$c,$ext,$file);
			}
...
sub addImageToDB {
  ...
  my ($image,$sc,$sql,$sql1,$sql2,@row);
	if (-e $fimg) {
    readFile($fimg,\$image);
    $sql1 = $dbh->quote($image);
    # calculate the unique id
    $sc  = $akte * 1000 + $seite;
    $sql = "select Seite from $db1.archivbilder where Seite=$sc";
