...
sub new {
  ...
  if ($self->param($self->EXIT_PARAM)>0) {
    ...
    if ($self->cookie) {
		  my ($host,$db,$user,$pw) = $self->getpw($self->cookie);
			if ($host eq "" || $db eq "" || $user eq "" || $pw eq "") {
			  if ($self->param("pw") eq $pw1) {
				  $self->addpw($self->cookie,$host1,$db1,$user1,$self->param("pw"));
				} else {
...
=head1
      ...
      if ($self->ses->isArchivistaMain) {
        ...
        if ($rec[0] ne $self->cookie) {
          $self->ses->addfile($self->ses->FLD_SESSIONID,$self->cookie,
...
sub addpw {
  my ($self,$cookie,$host,$db,$user,$pw) = @_;
  open(FOUT,">/tmp/$cookie");
	print FOUT "$host\t$db\t$user\t$pw\n";
	close(FOUT);
}
